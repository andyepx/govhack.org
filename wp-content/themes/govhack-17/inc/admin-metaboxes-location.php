...
function gh_location_meta_box($object)
{
    ...
    <p>
        <input type="text" id="venue_host" name="venue_host"
    ...
    <p>
        <input type="text" id="venue_team" name="venue_team"
               value="<?php echo esc_attr(get_post_meta($object->ID, 'venue_team', true)); ?>">
    </p>

    <p><label for="venue_accessibility"><?php _e("Accessibility", 'sequential'); ?></label> <code>venue_accessibility</code>
    </p>
    <p><textarea id="venue_accessibility"
    ...
    <p>
        <input type="number" id="venue_capacity" name="venue_capacity"
    ...
    <p><textarea id="venue_public_transport"
    ...
    <p><textarea id="venue_public_transport_last"
    ...
    <p>
        <input class="widefat" type="text" name="map_latlong" id="gh-location-meta-latlong"
...
function gh_location_meta_box_save($post_id)
{
    ...
    if (isset($_POST['map_latlong'])) {
        ...
        update_post_meta($post_id, 'venue_host', sanitize_text_field($_POST['venue_host']));
        update_post_meta($post_id, 'venue_team', sanitize_text_field($_POST['venue_team']));
        update_post_meta($post_id, 'venue_accessibility', sanitize_text_field($_POST['venue_accessibility']));
        update_post_meta($post_id, 'venue_under_18', sanitize_text_field($_POST['venue_under_18']));
        update_post_meta($post_id, 'venue_capacity', sanitize_text_field($_POST['venue_capacity']));
        update_post_meta($post_id, 'venue_parking', sanitize_text_field($_POST['venue_parking']));
        update_post_meta($post_id, 'venue_public_transport', sanitize_text_field($_POST['venue_public_transport']));
        update_post_meta($post_id, 'venue_public_transport_last', sanitize_text_field($_POST['venue_public_transport_last']));
        update_post_meta($post_id, 'times', sanitize_text_field($_POST['times']));
        update_post_meta($post_id, 'catering', sanitize_text_field($_POST['catering']));
    }
