...
function generate_post_select($select_id, $post_type, $selected = 0)
{
    ...
    foreach ($posts as $post) {
        echo '<option ' . ($post->ID == $selected ? "selected" : "") . ' value="', $post->ID, '"', $selected == $post->ID ? ' selected="selected"' : '', '>', $post->post_title, '</option>';
    }
    echo '</select>';
}
...
function gh_sponsor_meta_box($object)
{
    ...
    <p><label for="gh-sponsor-meta-sponsor-link"><?php _e("Link to sponsor website", 'sequential'); ?></label>
        <!--<code>link_sponsor</code>--></p>
    <p><input class="widefat" type="text" name="link_sponsor" id="gh-sponsor-meta-sponsor-link"
              value="<?php echo esc_attr(get_post_meta($object->ID, 'link_sponsor', true)); ?>"
              placeholder="Fully qualified URL"/></p>
    ...
    <hr>
    <p>
        <label for="gh-sponsor-meta-region"><?php _e("Sponsored region (only useful if state/local sponsor)", 'sequential'); ?></label>
        <!--<code>sponsor_region</code>--></p>
    ...
    <hr>
    <p>
        <label for="gh-sponsor-meta-region"><?php _e("Location (only useful for local sponsor)", 'sequential'); ?></label>
        <!--<code>sponsor_region</code>--></p>
    <p><?php generate_post_select('sponsor_location_id', 'locations', get_post_meta($object->ID, 'sponsor_location_id', true));
        //    wp_dropdown_pages(['id' => 'gh-sponsor-meta-location', 'class' => 'widefat', 'name' => 'sponsor_location_id', 'child_of' => $region_parent_page_id, 'selected' => get_post_meta($object->ID, 'sponsor_location_id', true), 'show_option_none' => '-- N/A --'])
        ?>
    </p>
    ...
    if (isset($locations_by_region)): ?>
        <p class="howto">Current: <?php
            ...
            ?></p>
        <p>
            <select id="gh-sponsor-meta-portal-location-gids" name="sponsor_portal_location_gids[]" class="widefat"
                    multiple size="6">
    ...
    <?php else: ?>
        <p><input id="gh-sponsor-meta-portal-location-gids" type="text" name="sponsor_portal_location_gids"
                  placeholder="CSV list of location GIDs" class="widefat"
                  value="<?php echo esc_attr(implode(', ', $sponsor_portal_location_gids)); ?>"></p>
    <?php endif; ?>
    ...
    <p>
        <select id="gh-sponsor-meta-portal-type" name="sponsor_portal_type" class="widefat">
            <option value="">-- N/A --</option>
            <option value="corporate" <?php echo $portal_type_corporate ? 'selected="selected"' : '' ?>>Corporate
            </option>
            <option value="government" <?php echo $portal_type_government ? 'selected="selected"' : ''; ?>>Government
            </option>
            <option value="community" <?php echo $portal_type_community ? 'selected="selected"' : '' ?>>Community
            </option>
        </select> <br>

        <input type="text" name="sponsor_portal_type-manual" placeholder="... or manually define a sponsor portal type"
               class="widefat">
    </p>
...
function gh_sponsor_meta_box_save($post_id)
{
    ...
    if (!empty($_POST['sponsor_location_id'])) {
        update_post_meta($post_id, 'sponsor_location_id', sanitize_text_field($_POST['sponsor_location_id']));
    }
